...
const MRPL_DEVICE_DATA = {
    ...
    gates: {
        ...
        'Phase 3 Gate': { cameras: '42/42', anpr: 3, bollards: 2, status: 'ONLINE' },
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('anprStatus').textContent = '95%';
    document.getElementById('frStatus').textContent = '94%';
});

function initializeMRPLData() {
    console.log('✅ Loading MRPL Device Data from BOQ');
    document.getElementById('totalCameras').textContent = MRPL_DEVICE_DATA.totalCameras;
    document.getElementById('onlineCameras').textContent = MRPL_DEVICE_DATA.onlineCameras;
    document.getElementById('offlineCameras').textContent = MRPL_DEVICE_DATA.offlineCameras;
    document.getElementById('anprStatus').textContent = '95%';
...
function initializeCharts() {
    ...
    if (gateMovementCtx) {
        new Chart(gateMovementCtx, {
            type: 'bar',
            data: {
                labels: ['Main Gate', 'Cargo Gate', 'LP Gate', 'Phase 3 Gate', 'Railway Siding'],
                datasets: [
                    {
                        label: 'Entries',
